...
#   timestamp: 2025-12-23T07:46:00+00:00
...
class ModelSpec(BaseModel):
    ...
    max_tokens: Optional[int] = Field(
        None, description="Maximum number of tokens to generate", examples=[4096], ge=1
    )
    ...
    timeout: Optional[int] = Field(
        None, description="Maximum execution time in seconds for the API request", ge=1
    )
...
class KnowledgeGraphConfig(BaseModel):
    language: Optional[str] = Field(
        ...
        examples=["The same language like input text"],
    )
...
class Node(BaseModel):
    ...
    type: Type1 = Field(..., description="Type of node", examples=["vector_search"])
...
class WorkflowStyle(BaseModel):
    ...
    layoutDirection: Optional[LayoutDirection] = None
...
class PaginatedResponse(BaseModel):
    total: Optional[int] = Field(
        None, description="Total number of items", examples=[100], ge=0
    )
    page: Optional[int] = Field(
        None, description="Current page number", examples=[1], ge=1
    )
    page_size: Optional[int] = Field(
        None, description="Number of items per page", examples=[10], ge=1
    )
    total_pages: Optional[int] = Field(
        None, description="Total number of pages", examples=[10], ge=1
    )
...
class TitleGenerateRequest(BaseModel):
    max_length: Optional[int] = Field(
        20, description="Maximum length of the generated title", ge=6, le=50
    )
    language: Optional[Language] = Field(
        "zh-CN", description="Language for the title generation (IETF BCP 47 tag)"
    )
    turns: Optional[int] = Field(
        1, description="Number of most recent conversation turns to consider", ge=1
    )
...
class FulltextIndexStatus(Enum):
    PENDING = "PENDING"
    CREATING = "CREATING"
    ACTIVE = "ACTIVE"
    DELETING = "DELETING"
...
class Document(BaseModel):
    ...
    vector_index_status: Optional[VectorIndexStatus] = None
    fulltext_index_status: Optional[FulltextIndexStatus] = None
    graph_index_status: Optional[GraphIndexStatus] = None
    summary_index_status: Optional[SummaryIndexStatus] = None
    vision_index_status: Optional[VisionIndexStatus] = None
...
class SearchResultItem(BaseModel):
    ...
    recall_type: Optional[RecallType] = Field(None, description="Recall type")
...
class NodeMergeResponse(BaseModel):
    ...
    redirected_edges: int = Field(
        ...,
        description="Number of edges that were redirected during merge",
        examples=[12],
        ge=0,
    )
    merged_description_length: int = Field(
        ..., description="Length of the merged description", examples=[512], ge=0
    )
...
class MergeSuggestionsRequest(BaseModel):
    ...
    max_concurrent_llm_calls: Optional[int] = Field(
        4,
...
class MergeSuggestionsResponse(BaseModel):
    ...
    total_analyzed_nodes: int = Field(
        ..., description="Total number of nodes analyzed", examples=[156], ge=0
    )
    processing_time_seconds: float = Field(
        ..., description="Processing time in seconds", examples=[12.5], ge=0.0
    )
    ...
    total_suggestions: int = Field(
        ..., description="Total number of suggestions", examples=[5], ge=0
    )
    pending_count: int = Field(
        ..., description="Number of pending suggestions", examples=[3], ge=0
    )
    accepted_count: int = Field(
        ..., description="Number of accepted suggestions", examples=[1], ge=0
    )
    rejected_count: int = Field(
        ..., description="Number of rejected suggestions", examples=[1], ge=0
    )
...
class SharedCollectionConfig(BaseModel):
    enable_vector: Optional[bool] = Field(
        True, description="Whether vector search is enabled"
    )
    enable_fulltext: Optional[bool] = Field(
        True, description="Whether fulltext search is enabled"
    )
    enable_knowledge_graph: Optional[bool] = Field(
        True, description="Whether knowledge graph is enabled"
    )
    enable_summary: Optional[bool] = Field(
        False, description="Whether summary generation is enabled"
    )
    enable_vision: Optional[bool] = Field(
        False, description="Whether vision processing is enabled"
    )
...
class LlmConfigurationResponse(BaseModel):
    providers: Optional[list[LlmProvider]] = Field(
        [], description="List of LLM providers"
    )
    models: Optional[list[LlmProviderModel]] = Field(
...
class LlmProviderModelCreate(BaseModel):
    ...
    api: Api = Field(..., description="API type for this model")
...
class EmbeddingData(BaseModel):
    object: Optional[str] = Field(
        "embedding", description="Object type identifier", examples=["embedding"]
    )
...
class EmbeddingResponse(BaseModel):
    object: Optional[str] = Field(
        "list", description="Object type identifier", examples=["list"]
    )
...
class QuotaUpdateRequest(BaseModel):
    max_collection_count: Optional[int] = Field(
        None, description="New limit for collection count", ge=0
    )
    max_document_count: Optional[int] = Field(
        None, description="New limit for document count", ge=0
    )
    max_document_count_per_collection: Optional[int] = Field(
        None, description="New limit for documents per collection", ge=0
    )
    max_bot_count: Optional[int] = Field(
        None, description="New limit for bot count", ge=0
    )
...
class SystemDefaultQuotas(BaseModel):
    max_collection_count: int = Field(
        ..., description="Default maximum collection count", examples=[10], ge=0
    )
    max_document_count: int = Field(
        ..., description="Default maximum document count", examples=[1000], ge=0
    )
    max_document_count_per_collection: int = Field(
        ...,
        description="Default maximum documents per collection",
        examples=[100],
        ge=0,
    )
    max_bot_count: int = Field(
        ..., description="Default maximum bot count", examples=[5], ge=0
    )
...
class WebReadResultItem(BaseModel):
    ...
    status: Status11 = Field(..., description="Processing status")
...
class EvaluationItemStatus(Enum):
    PENDING = "PENDING"
    RUNNING = "RUNNING"
    COMPLETED = "COMPLETED"
    FAILED = "FAILED"
